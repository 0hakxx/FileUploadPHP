...
<?
  ...
  if(empty($menu)) {
    echo "<script>alert(\"잘못된 접근 입니다.\");history.back(-1);</script>";
    exit;
  }
  ...
  if($mode == "write") {
    $query = "insert into {$tableName1} (title, writer, content, regdate, file, gubun, password) values ('{$title}', '{$writer}', '{$content}', now(), '{$file}', '{$gubun}_{$menu}', '{$password}')";
    $db_conn->query($query);
  } else if($mode == "modify") {
    if(!is_numeric($idx)){
    ...
    $query = "update {$tableName1} set title='{$title}', writer='{$writer}', content='{$content}', regdate=now(), file='{$file}' where idx={$idx}";
    $db_conn->query($query);
  } else if($mode == "delete") {
    if(!is_numeric($idx)){
        echo "<script>alert(\"올바르지 않은 값이 입력 되었습니다.\");history.back(-1);</script>";
