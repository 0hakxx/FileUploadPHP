...
<?
		...
		if (@getimagesize($_FILES["file"]["tmp_name"]) !== false) {
			...
			if(10 < $size[0] && 10 < $size[1] && $size[0] <= 50 && $size[1] <= 50) {
				...
				echo("<div class=\"alert alert-danger\" role=\"alert\">* <strong>업로드 실패!</strong> 이미지 가로/세로 사이즈를 10px ~ 50px로 해주세요.</div><center><button type=\"button\" class=\"btn btn-secondary btn-sm\" onclick=\"history.back(-1)\">← 뒤로가기</button></center>");
				exit;
			}
			
		} else {
			echo("<div class=\"alert alert-danger\" role=\"alert\">* <strong>업로드 실패!</strong> 이미지 파일이 아닙니다.</div><center><button type=\"button\" class=\"btn btn-secondary btn-sm\" onclick=\"history.back(-1)\">← 뒤로가기</button></center>");
			exit;
		}
